...
func TestSelect(t *testing.T) {
	for _, tt := range []struct {
		...
		stmt:  Select(NewCallExpr("unnest", NewIndexExpr("logs", NewBindValue(1)))),
		query: "SELECT unnest(logs[$1:])",
		args:  []interface{}{1},
	},
		{
			stmt: Select("col").From("table").Where(
				NewBoolExpr(NewBoolExpr(
					NewIdentifier("deleted_at").IsNull()).
					And(NewIdentifier("id").LessThan(1))).
					And(NewIdentifier("status").In([]string{"success", "failed"})),
			),
